...
// from YouTube's capture-phase hotkeys, and exposes window.YTBTheme.icon().
...
/* --- Theme Preference overrides (ADR-0008) ---
   :root[data-theme=...] (0,2,0) out-specifies the bare :root (0,1,0) inside
   the prefers-color-scheme media query, so a forced mode wins over the OS in
   BOTH directions while 'system' (no attribute) falls through to the media
   query above. Each block re-asserts exactly the tokens the dark block
   flips. */
...
`;
	...
	// Pure marker decision lives in shared.js (YTB.themeMarker): forced
	// Light/Dark always win; under Auto it follows the page. shared.js loads
	// before theme.js in both contexts, so window.YTB is defined here.

	// Under Auto in the popup there's no page to follow, so the marker is left
	// unset (OS media query rules); on YouTube, `dark` on <html> is the source.
	...
	try {
		chrome.storage.local.get('theme').then(
			(stored) => applyTheme(stored && stored.theme),
			() => {},
		);
		chrome.storage.onChanged.addListener((changes, area) => {
			if (area !== 'local' || !changes.theme) return;
			// Guard against a stale (post-reload) content script touching the page.
			try {
		...
		// content.js emits this on a live YouTube dark-mode flip; re-decide the
		// marker (no-op in the popup, which never sees this event).
		...
		// Extension context already invalidated at load: leave the System default.
	}
	...
	const ICON_PATHS = {
		...
		// Dismiss cross.
		close:
			'M18.3 5.7a1 1 0 0 0-1.4 0L12 10.6 7.1 5.7a1 1 0 0 0-1.4 1.4l4.9 4.9-4.9 4.9a1 1 0 1 0 1.4 1.4l4.9-4.9 4.9 4.9a1 1 0 0 0 1.4-1.4L13.4 12l4.9-4.9a1 1 0 0 0 0-1.4Z',
	};
	...
	// YouTube's player captures hotkeys before any bubble listener runs (space
	// toggled play, Enter re-toggled the Spoiler checkbox); swallow the event
	// at window capture and replay it as `ytb:keydown` (a cloned KeyboardEvent
	// would retrace the same capture path and still reach YouTube - the new
	// event name is what hides it). Typing and the checkbox default stay
	// native since the real event's default action is left alone.
	...
	for (const type of ['keydown', 'keyup', 'keypress']) {
		window.addEventListener(
			type,
			(event) => {
				if (!event.isTrusted) return; // synthetic key events are not ours to intercept
