...
**Note**:
A Buddy's timestamped comment or emoji, pinned to one moment in one video, visible to the whole Room (like Progress Records). One entity with two content kinds: a text Note (limited to 100 characters), or an emoji **Reaction** (chosen from a small curated set, parallel to the eight curated Buddy Colors). Distinct from a Buddy's position **Progress Marker** (a Progress Marker = live watch position; a Note = authored content left at a moment). A text Note may be marked a **Spoiler**, showing only an obscured dot on the timeline until the viewer's own playhead passes its timestamp; while locked, it can be _previewed_ (author and timestamp shown, body reads "Spoiler") but still cannot be _expanded_. Clicking a locked Spoiler dot performs **Go here** (seeks to just before its timestamp and resumes playback) instead of doing nothing, so the viewer can jump to the moment and let the Note reveal naturally as the playhead crosses it. Clicking a Reaction dot is a bare, state-preserving seek to just before its timestamp — playing stays playing, paused stays paused (not Go here, which resumes playback). Reactions are never Spoilers.
...
**Recommend Control**:
The control a member uses to add a Recommendation, in two placements that share one behavior: the watch-page "+ Buddy Room" pill and the feed thumbnail's three-dots menu row. Adding recommends the video to the whole Room (see Recommendation); on the watch page the same control exposes **un-recommend** for the member's own videos. Recommend Control writes are NOT gated by Sharing.
_Avoid_: add-to-playlist button, Buddy Room button (as the canonical name), save button
