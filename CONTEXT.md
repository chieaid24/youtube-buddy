...
**Control Panel**:
The extension's action popup — `popup.html`. It holds identity (the Nickname / Display Name field), the Room Code entry with the current Room state, the per-Buddy list (each with its Buddy Color Swatch), and the click-to-toggle Sharing Dot. It has **two entry points to the same document**: the toolbar icon (Chrome renders it as the action popup), and the **Control Panel Launcher** — a small control-knobs (sliders) icon at the right end of the Room Home Toggle row in the left guide that opens `popup.html` in an in-page overlay (an extension-origin iframe, so it stays the Control Panel document, not a native in-page surface). It is still distinct from every in-page surface — the Video Timeline, the Note Composer, and the Room Home Section. All state it reads and writes lives in `chrome.storage.local`, so a change made in either entry point propagates to every open surface via `chrome.storage.onChanged`.
...
A small control-knobs (sliders) icon at the right end of the Room Home Toggle row (in YouTube's left guide) that opens the full **Control Panel** in an in-page overlay (an extension-origin iframe of `popup.html`) without leaving the YouTube tab. The icon is deliberately not a gear (it opens the whole hub, not just Settings). Purely a second entry point to the existing Control Panel — it renders no new controls. It sits in the toggle row, **not** inside the Room Home Section, so it stays reachable even when the section is hidden; clicking the icon opens the panel and does not toggle the section (the rest of the row still toggles). Lives only on the home route (where the guide row exists); the toolbar icon remains the entry point everywhere else.
_Avoid_: settings button (opens the whole Control Panel, not just Settings), in-page popup
...
**Watched-By Dots**:
The small cluster of flat Buddy-colored dots overlaid in the top-left of a feed thumbnail's image box — one dot per Buddy who has a Progress Record for that video, ordered most-recent-first, the viewer excluded (YouTube's own red **Watched Bar** already tells the viewer's state). A dot means only "this Buddy has a record for this video": no fraction, no timestamp, no started-vs-watched distinction — the thumbnail-surface counterpart to the Progress Marker (which sits on the active Video Timeline and does carry a position). Hovering or keyboard-focusing the cluster shows a single dark "Watched by <names>" tooltip (Buddies-only, collapsing to "and N other(s)" after two names). Kept player-native, not part of the apricot Note UI. The dots are drawn inside the thumbnail's own box, never overhang the image, and stay visible and hoverable while the tile plays YouTube's hover-autoplay inline preview. Distinct from a **Note Dot** (authored content on the Video Timeline) and the timeline **Dot Cluster** (Note Dots fanning apart on hover). Replaced the retired thumbnail Progress Bar (a fractional per-Buddy band strip).
...
**Post Echo**:
The Playback Notification the author fires for themselves the instant a Note or Reaction is posted from the Note Composer — the same card or burst a Buddy sees, in the same container at the same Notification Position, authored "You". It is a write receipt AND a preview of what the Room gets: the author sees exactly the presentation their Buddies will. It does not depend on playback, so it shows while the video is paused (the Note Composer pauses on open, so this is the common case). It fires once: the crossing window rebases to the new Note's timestamp, so resuming playback past that moment does not immediately repeat it — a later rewind-and-replay across the timestamp fires normally, like any other Note. Replies have no Post Echo (the Expanded Note that sent one already shows it).
