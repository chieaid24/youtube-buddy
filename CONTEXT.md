...
The floating panel the extension overlays on the YouTube **home page** (`youtube.com`): a `position: fixed`, left-docked, portrait overlay appended to `<body>` over a slightly-dim scrim, holding the Room Feed stacked ABOVE the Recommended for you list (no left/right columns). It reads no YouTube layout DOM and never shifts the recommendations grid. It is a distinct surface from the action popup and the Video Timeline (see ADR-0005). When Unpaired it shows a compact Create/Join prompt instead of the Feed + Recommendations. It is **ephemeral**: the Room Home Toggle OPENS it, and its header close control (an X at the right edge), a scrim click, Esc, or an SPA navigation all CLOSE it — there is no persisted visibility preference. The scrim BLOCKS the page while the panel is open (accepted: it opens on demand and closes on navigation, so the page is never left blocked). Closing is a hide, never a leave: no Room membership, Sharing state, or Recommendation is touched, and the Room Home Toggle reopens it.
_Avoid_: home widget, dashboard, feed (alone)

**Room Home Toggle**:
An on/off control injected as a row into YouTube's own left guide (sidebar) on the home route, letting the viewer open or close the Room Home Panel. It is styled as a native guide entry — icon + "Buddy Room" label, pixel-matched to the sibling rows (Home, Shorts, Subscriptions) — with no separate switch: the buddies icon itself is the state indicator, tinted apricot while the panel is open and the native guide grey while closed. The row itself always stays in the guide on the home route (it also hosts the Control Panel Launcher). It persists nothing: clicking it dispatches an open/close request and mirrors the panel's live open state — the panel owns that state and broadcasts it via an in-page CustomEvent, so there is no storage round-trip and no matching control in the popup. Toggling only affects the Room Home Panel — on-video markers, Notes, presence, and the watch-page recommend control keep running (the point is to open the home surface without disabling the extension).
